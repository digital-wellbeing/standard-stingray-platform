...
// If task is undefined or empty, run all tasks
var tasksToRun;
if (typeof task === "undefined" || task === "") {
    tasksToRun = ["stroop", "flanker", "simon"];
} else {
    // Split by comma and trim any whitespace; also convert to lower case
    tasksToRun = task.split(",").map(function(x) { return x.trim().toLowerCase(); });
}

// Loop through the list and push the corresponding tasks to the timeline
tasksToRun.forEach(function(taskName) {
    if (taskName === "stroop") {
        timeline.push(stroop_task, save_data_stroop);
    } else if (taskName === "flanker") {
        timeline.push(flanker_task, save_data_flanker);
    } else if (taskName === "simon") {
        timeline.push(simon_task, save_data_simon);
    }
});
